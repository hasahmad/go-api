...
func NewTokenStore(db *sqlx.DB, options ...TokenStoreOption) (*TokenStore, error) {
	store := &TokenStore{
		...
		logger:     log.New(os.Stderr, "[OAUTH2-sqlx-ERROR]", log.LstdFlags),
...
func (s *TokenStore) clean() {
	now := time.Now()
	_, err := s.db.ExecContext(context.Background(), fmt.Sprintf("DELETE FROM %s WHERE expires_at <= $1", s.tableName), now)
	if err != nil {
...
func (s *TokenStore) RemoveByCode(ctx context.Context, code string) error {
	_, err := s.db.ExecContext(ctx, fmt.Sprintf("DELETE FROM %s WHERE code = $1", s.tableName), code)
	if err == nil {
		return nil
	}
	return err
...
func (s *TokenStore) RemoveByAccess(ctx context.Context, access string) error {
	_, err := s.db.ExecContext(ctx, fmt.Sprintf("DELETE FROM %s WHERE access = $1", s.tableName), access)
	if err == nil {
		return nil
	}
	return err
...
func (s *TokenStore) RemoveByRefresh(ctx context.Context, refresh string) error {
	_, err := s.db.ExecContext(ctx, fmt.Sprintf("DELETE FROM %s WHERE refresh = $1", s.tableName), refresh)
	if err == nil {
		return nil
	}
	return err
...
func (s *TokenStore) GetByCode(ctx context.Context, code string) (oauth2.TokenInfo, error) {
	...
	if err := s.db.GetContext(ctx, &item, fmt.Sprintf("SELECT * FROM %s WHERE code = $1", s.tableName), code); err != nil {
		return nil, err
	}
...
func (s *TokenStore) GetByAccess(ctx context.Context, access string) (oauth2.TokenInfo, error) {
	...
	var item TokenStoreItem
	if err := s.db.GetContext(ctx, &item, fmt.Sprintf("SELECT * FROM %s WHERE access = $1", s.tableName), access); err != nil {
		return nil, err
	}
...
func (s *TokenStore) GetByRefresh(ctx context.Context, refresh string) (oauth2.TokenInfo, error) {
	...
	var item TokenStoreItem
	if err := s.db.GetContext(ctx, &item, fmt.Sprintf("SELECT * FROM %s WHERE refresh = $1", s.tableName), refresh); err != nil {
		return nil, err
	}
