...
type OfficeRequestsRepo struct {
	DB  *sqlx.DB
	sql *goqu.Database
}

func (r OfficeRequestsRepo) TableName() string {
	return "office_requests"
}

func (r OfficeRequestsRepo) PrimaryKey() string {
	return "office_request_id"
}

func (r OfficeRequestsRepo) FindAll(ctx context.Context, wheres []goqu.Expression, f *filters.Filters) ([]models.OfficeRequest, error) {
...
func (r OfficeRequestsRepo) Update(ctx context.Context, id uuid.UUID, data helpers.Envelope) (models.OfficeRequest, error) {
	sel := r.sql.
		Update(r.TableName()).
		Set(data).
...
func (r OfficeRequestsRepo) Delete(ctx context.Context, id uuid.UUID) error {
	sel := r.sql.
		Delete(r.TableName()).
		Where(goqu.Ex{r.PrimaryKey(): id}).
		Limit(1)
